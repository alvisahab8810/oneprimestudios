...
export default function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const transformedAttributes = form.attributes.map((attr) => {
        const values =
          Array.isArray(attr.values) && attr.values.length > 0 && typeof attr.values[0] === "object"
            ? attr.values
  ...
  // attribute helpers
  const addAttribute = () =>
    setForm((f) => ({ ...f, attributes: [...f.attributes, { name: "", type: "select", valuesCSV: "", required: false }] }));
  ...
  const removeAttribute = (idx) => setForm((f) => ({ ...f, attributes: f.attributes.filter((_, i) => i !== idx) }));
  ...
  return (
    ...
    //         <p >
    //           CSV format: <code>Label:modifier</code>, e.g. <code>Single-Sided:0,Double-Sided:5</code>
    //         </p>

    //         {form.attributes.map((attr, i) => (
    //           <div key={i}>
    //             <div>
    //               <input placeholder="Attribute Name" value={attr.name} onChange={(e) => updateAttribute(i, { name: e.target.value })}  />
    //               <select value={attr.type} onChange={(e) => updateAttribute(i, { type: e.target.value })}>
    ...
    //               </select>
    //               <label>
    //                 <input type="checkbox" checked={!!attr.required} onChange={(e) => updateAttribute(i, { required: e.target.checked })} /> Required
    //               </label>
    //               <button type="button" onClick={() => removeAttribute(i)}>Delete</button>
    //             </div>

    //             <input placeholder="e.g. Single-Sided:0,Double-Sided:5" value={attr.valuesCSV || attr.values?.map((v) => `${v.label}:${v.priceModifier}`).join(",") || ""} onChange={(e) => updateAttribute(i, { valuesCSV: e.target.value })} />
    //           </div>
    //         ))}
    ...
    //           <input name="basePrice" value={form.basePrice} onChange={handleChange} type="number" required  />
    //           <label>Sale Price</label>
    //           <input name="salePrice" value={form.salePrice} onChange={handleChange} type="number" />
    //         </div>
    ...
    //           <input name="minOrderQty" value={form.minOrderQty} onChange={handleChange} type="number" />
    //           <label >
    ...
    //         <input placeholder="e.g. 1:10,50:8,100:6" value={form.pricingTiersCSV || ""} onChange={(e) => setForm((f) => ({ ...f, pricingTiersCSV: e.target.value }))} />
    ...
    //           <input placeholder="e.g. 50,100,200" value={form.b2b_quantityOptionsCSV} onChange={(e) => setForm((f) => ({ ...f, b2b_quantityOptionsCSV: e.target.value }))} />
    //         </label>
    ...
    //         <label >
    //           (optional) B2C fixed quantity options (comma separated)
    //           <input placeholder="e.g. 1,2,5" value={form.b2c_quantityOptionsCSV} onChange={(e) => setForm((f) => ({ ...f, b2c_quantityOptionsCSV: e.target.value }))}  />
    //         </label>
    //       </div>
  ...
  <section className="category-section">
    ...
    <div className="category-inputs">
      <select name="categoryId" value={form.categoryId} onChange={handleChange} className="category-select">
        <option value="">Select category</option>
        {categories.map((c) => (
          <option key={c._id} value={c._id}>{c.name}</option>
        ))}
      </select>
      <input className="category-new-input" placeholder="New category name" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
      <button type="button" className="btn btn-add-category" onClick={createCategory}>Add Category</button>
    </div>
  </section>
    ...
    <div className="form-grid">
      ...
      <div className="form-left">
        ...
        <div className="short-desc-row">
          <input name="shortDescription" placeholder="Short Description" value={form.shortDescription} onChange={handleChange} className="input-primary" />
        ...
        <div className="attributes-section">
          <h4>Custom Attributes</h4>
          <p className="note">CSV format: <code>Label:modifier</code>, e.g. <code>Single-Sided:0,Double-Sided:5</code></p>
          ...
          {form.attributes.map((attr, i) => (
            <div className="attribute-card" key={i}>
              <div className="attribute-row">
                ...
                <button type="button" className="delete-btn" onClick={() => removeAttribute(i)}>Delete</button>
              </div>
              <input placeholder="e.g. Single-Sided:0,Double-Sided:5" className="attribute-values" value={attr.valuesCSV || attr.values?.map((v) => `${v.label}:${v.priceModifier}`).join(",") || ""} onChange={(e) => updateAttribute(i, { valuesCSV: e.target.value })} />
            </div>
          ))}

          <button type="button" className="btn add-attribute-btn" onClick={addAttribute}>➕ Add Attribute</button>
        </div>
        ...
        <div className="b2b-section">
          ...
          <label><input type="checkbox" name="b2b_enabled" checked={form.b2b_enabled} onChange={handleChange} /> Enable B2B</label>
          <label><input type="checkbox" name="b2b_allowFileUpload" checked={form.b2b_allowFileUpload} onChange={handleChange} /> Allow file upload</label>
          <input placeholder="Quantity options (comma separated)" value={form.b2b_quantityOptionsCSV} onChange={(e) => setForm((f) => ({ ...f, b2b_quantityOptionsCSV: e.target.value }))} className="input-primary" />
        </div>
        ...
        <div className="b2c-section">
          ...
          <label><input type="checkbox" name="b2c_designUpload" checked={form.b2c_designUpload} onChange={handleChange} /> Allow design upload</label>
          <label><input type="checkbox" name="b2c_whatsapp" checked={form.b2c_whatsapp} onChange={handleChange} /> WhatsApp support</label>
          <input placeholder="Fixed quantity options (comma separated)" value={form.b2c_quantityOptionsCSV} onChange={(e) => setForm((f) => ({ ...f, b2c_quantityOptionsCSV: e.target.value }))} className="input-primary" />
        </div>
      </div>
