...
export default async function getUserFromToken(maybeHeaderOrReq) {
  try {
    ...
    // NEW: if partner account has been revoked, treat as unauthorized immediately
    // This ensures every API call returns 401 the instant admin revokes the partner,
    // without waiting for token expiry.
    if (user.userType === "partner" && user.isApproved === false) return null;
